...
ctrl.g = 9.81;
ctrl.k_sea = [3343; 3825; 3476; 3905];
ctrl.l_seg = 0.50;
...
% time after first stance phase to start control
ctrl.control_start_time = 10.0;
% initial primary leg: 1 == right, 0 == left (always start with left leg on boom)
ctrl.initial_leg_selection = 0; 
% double support static targets
ctrl.static_double_x_pos_swing = -1.1*cosd(75);
ctrl.static_double_z_pos_swing = 1.1*sind(75);
ctrl.static_double_x_pos_stance = ctrl.static_double_x_pos_swing;
ctrl.static_double_z_pos_stance = ctrl.static_double_z_pos_swing;
% single support static targets
ctrl.static_single_x_pos_swing = 0;
ctrl.static_single_z_pos_swing = 1.1 - 0.04;
ctrl.static_single_x_pos_stance = ctrl.static_single_x_pos_swing;
ctrl.static_single_z_pos_stance = ctrl.static_single_z_pos_swing;
...
ctrl.kp_lateral_swing = 900;
ctrl.kd_lateral_swing = 0.7*2*sqrt(ctrl.kp_lateral_swing*ctrl.i_lateral_about_pelvis);
...
ctrl.Q_lqr_single = diag([1, 10, 100, 100, 150, 10]);
ctrl.H_lqr_single = diag([1, 10, 100, 100, 150, 10]);
ctrl.R_lqr_single = diag([1.5e-5 3e2]);
%ctrl.R_lqr_single = diag([3e-5 4e2]);

ctrl.Q_lqr_double = diag([1, 10, 100, 100, 150, 10]);
ctrl.H_lqr_double = diag([1, 10, 100, 100, 150, 10]); 
ctrl.R_lqr_double = diag([10, 1, 1]*5e-4);
...
ctrl.stop_stance_leg_extension = 0.94; % leg length to start reducing ground forces
...
% boom compensation
ctrl.fz_boom_compensation_right = 1.0;
ctrl.fz_boom_compensation_left = 0.86;

%% CoM Motion Plans
ctrl.planner_sample_time = 0.008;
rest = extract_policy_variables('policies/rest.mat');
walk_slow = extract_policy_variables('policies/06_walk.mat');
%walk_slow = extract_policy_variables('policies/10_walk.mat');
run_slow = extract_policy_variables('policies/06_run_reduced.mat');
%run_fast = extract_policy_variables('policies/12_run.mat');
%run_fast = extract_policy_variables('policies/18_run.mat');

%% Low-pass filters
ctrl.lpf_damping = sqrt(2)/2;
ctrl.f_imu = 250*(2*pi);            % cutoff for orientation velocities
ctrl.f_accelerometer = 200*(2*pi);  % cutoff for translational accelerations
ctrl.f_lateral_accel = 200*(2*pi);  % cutoff for lateral angular accelerations
ctrl.f_dtau = 200*(2*pi);           % cutoff for derivative of desired joint torques
ctrl.f_tau = 200*(2*pi);            % cutoff for desired sea torques
...
ctrl.B1_lpf_tau = -2*exp(-ctrl.lpf_damping*ctrl.f_tau*sample_time)*...
