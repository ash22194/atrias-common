% plot data logs downloaded from ATRIAS
%% Load data into workspace
t_start = 10;
t_select_1000 = time_1000_recorded>=t_start & time_1000_recorded<=time_1000_recorded(end);
t_select_4000 = time_4000_recorded>=t_start & time_4000_recorded<=time_4000_recorded(end);
q = q_recorded(t_select_1000,:);
dq = dq_recorded(t_select_1000,:);
tau_lateral = lateral_torque_commands_recorded(t_select_1000,:);
desired_sea_torques = desired_sea_torques_recorded(t_select_1000,:);
qinc = elmo_measured_positions_recorded(t_select_4000,:);
dqinc = elmo_measured_velocities_recorded(t_select_4000,:);
velocity_command = elmo_commanded_velocities_recorded(t_select_4000,:);
elmo_velocity = elmo_measured_velocities_recorded(t_select_4000,:);
elmo_torque = elmo_measured_torques_recorded(t_select_4000,:);
elmo_velocity_demands = elmo_demanded_velocities_recorded(t_select_4000,:);
elmo_torque_demands = elmo_demanded_torques_recorded(t_select_4000,:);
motors_enabled = motors_enabled_recorded(t_select_4000,:);
motors_on = [0; diff(motors_enabled)] == 1;
motors_off = [diff(motors_enabled); 0] == -1;
t_1000 = time_1000_recorded(t_select_1000);
t_4000 = time_4000_recorded(t_select_4000);
fprintf('Motors enabled at t=%f s\n',t_4000(motors_on));
fprintf('Motors disabled at t=%f s\n',t_4000(motors_off));

%% Start plotting
% Leg Segment Positions
plot_fscope('Leg Position',{t_1000,t_1000}, {q(:,1:4)*180/pi, q(:,5:8)*180/pi},...
...
plot_fscope('Leg Velocity',{t_1000,t_1000}, {dq(:,1:4)*180/pi, dq(:,5:8)*180/pi},...
...
plot_fscope('Virtual Leg Length',{t_1000,t_1000}, {[right_leg_length right_leg_length_using_motors], [left_leg_length left_leg_length_using_motors]},...
    {{'Measured','Using Motor Position'},{'Measured','Using Motor Position'}},...
    {'Right Leg Length','Left Leg Length'},{'Time (sec)','Time (sec)'},...
    {'Length (m)','Length (m)'},{'auto','auto'},2);
% Virtual Leg Angle
right_leg_angle = (q(:,1)+q(:,3))/2;
left_leg_angle = (q(:,5)+q(:,7))/2;
right_leg_angle_velocity = (dq(:,1)+dq(:,3))/2;
left_leg_angle_velocity = (dq(:,5)+dq(:,7))/2;
plot_fscope('Virtual Leg Angle',{t_1000,t_1000}, {right_leg_angle*180/pi, left_leg_angle*180/pi},...
    {{'Measured'},{'Measured'}},...
    {'Right Leg Angle','Left Leg Angle'},{'Time (sec)','Time (sec)'},...
    {'Angle (degrees)','Angle (degrees)'},{'auto','auto'},2);
% Lateral Positions)
plot_fscope('Lateral Position',{t_1000,t_1000}, {[(q(:,9:10))*180/pi], [dq(:,9:10)*180/pi]},...
...
plot_fscope('SEA Torques',{t_1000,t_1000}, {[spring_forces(:,1:2), desired_sea_torques(:,1:2)], [spring_forces(:,3:4), desired_sea_torques(:,3:4)]}, ...
...
plot_fscope('Lateral Torques',{t_1000}, {lateral_torque_commands}, ...
    {{'Right Commanded','Left Commanded'}},...
    {'Lateral Torques'}, {'Time (sec)'}, {'Torque (Nm)'}, {'auto'}, 1);
% leg force
...
plot_fscope('Leg Forces',{t_1000,t_1000}, {[right_leg_force_measured], [left_leg_force_measured]}, ...
    {{'Measured'},{'Measured'}},...
    {'Right Leg Force','Left Leg Force'}, {'Time (sec)','Time (sec)'}, {'Force (N)','Force (N)'}, {'auto','auto'}, 2);
% leg torque
    right_leg_torque_measured = (spring_forces(:,1)+spring_forces(:,2));
    left_leg_torque_measured = (spring_forces(:,3)+spring_forces(:,4));
plot_fscope('Leg Torques',{t_1000,t_1000}, {[right_leg_torque_measured], [left_leg_torque_measured]}, ...
    {{'Measured'},{'Measured'}},...
    {'Right Leg Torque','Left Leg Torque'}, {'Time (sec)','Time (sec)'}, {'Torque (Nm)','Torque (Nm)'}, {'auto','auto'}, 2);
% Velocity commands
plot_fscope('Motor Velocity',{t_4000,t_4000}, {[velocity_command(:,1:2), elmo_velocity_demands(:,1:2), elmo_velocity(:,1:2)], [velocity_command(:,3:4), elmo_velocity_demands(:,3:4), elmo_velocity(:,3:4)]}, ...
...
% Torque commands
plot_fscope('Motor Torque',{t_4000,t_4000}, {[elmo_torque(:,1:2), elmo_torque_demands(:,1:2)], [elmo_torque(:,3:4), elmo_torque_demands(:,3:4)]}, ...
