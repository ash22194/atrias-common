...
LEG_A_CALIB_LOC = (180+35)*pi/180; % Radians
LEG_B_CALIB_LOC = (180-35)*pi/180; % Radians
...
RIGHT_HIP_INC_ENCODER_RAD_PER_TICK =  2*pi / (2500 * 4 * 57);  % Ticks to rad constant = rad_per_rev / (lines_per_rev * quadrature * gear_ratio)
LEFT_HIP_INC_ENCODER_RAD_PER_TICK =  2*pi / (2500 * 4 * 57);  % Ticks to rad constant = rad_per_rev / (lines_per_rev * quadrature * gear_ratio)
...
RIGHT_HIP_CALIB_VAL = 5534;   %Calibration encoder value in ticks
LEFT_HIP_CALIB_VAL = 255;  %Calibration encoder value in ticks
...
MTR_MAX_CONT_CURRENT = 100.0; %Maximum continuous motor current
...
LEG_CURRENT_LIMIT = 140;
HIP_CURRENT_LIMIT = 60;


% From leg1_definitions.h (corresponds to CMU right leg)
%(Note that A=shin and B=thigh for OSU convention)
LEG1_LEG_A_CALIB_VAL = 227561909 - 103094; %Calibration encoder value in ticks
LEG1_LEG_B_CALIB_VAL = 228222924 - 2291; %Calibration encoder value in ticks
LEG1_LEG_A_RAD_PER_CNT = -9.8039216e-09; %Ticks to rad constant
LEG1_LEG_B_RAD_PER_CNT = 9.8039216e-09; %Ticks to rad constant
LEG1_TRAN_A_CALIB_VAL = 162479274; %Calibration encoder value in ticks
LEG1_TRAN_B_CALIB_VAL = 163654839; %Calibration encoder value in ticks
...
%(Note that A=shin and B=thigh for OSU convention)
LEG2_LEG_A_CALIB_VAL = 208820700 - 298095; %Calibration encoder value in ticks
LEG2_LEG_B_CALIB_VAL = 229265180 + 127895; %Calibration encoder value in ticks
LEG2_LEG_A_RAD_PER_CNT = 9.8039216e-09; %Ticks to rad constant
LEG2_LEG_B_RAD_PER_CNT = 9.8039216e-09; %Ticks to rad constant
LEG2_TRAN_A_CALIB_VAL = 162399822; %Calibration encoder value in ticks
LEG2_TRAN_B_CALIB_VAL = 177694970; %Calibration encoder value in ticks
...
BOOM_PITCH_CAL_VALUE_RAD = 0*pi/180;
BOOM_PITCH_CAL_VALUE_TICKS  = 73950; % ticks, pitch encoder reading when robot is vertical
BOOM_ROLL_CAL_VALUE_RAD = 11.2*pi/180; %Roll angle (in radians) for calibration point
BOOM_ROLL_CAL_VALUE_TICKS = 83345; %Roll encoder value for calibration point

% Soft limits for motor positions
limExt = 0.1;
MOTOR_POSITION_LIMITS_UPPER = [3.60-limExt; 4.78-limExt; 2.47-0.3]; % A B retraction
MOTOR_POSITION_LIMITS_LOWER = [1.51+limExt; 2.68+limExt; 0.50+0.0]; % A B extension
...
max_encoder_velocity = 250; % rad/s, max rate which encoder values can change
sensor_start_time = 9; % s
